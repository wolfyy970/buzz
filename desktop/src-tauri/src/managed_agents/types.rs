...
pub struct AgentDefinition {
    ...
    /// Latest immutable version published to Buzz-hosted Git. Saving the
    /// mutable template head does not change this reference.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub published_version: Option<AgentTemplateVersionRef>,
    /// Local values pinned alongside `published_version`.
    ///
    /// These values can contain credentials. They stay in the restricted
    /// local template store and are excluded from Git, Nostr events, portable
    /// snapshots, and frontend projections.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub published_version_env_vars: Option<BTreeMap<String, String>>,
...
pub struct ManagedAgentRecord {
    ...
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty")]
    pub env_vars: BTreeMap<String, String>,
    /// Buzz Project this agent works in. Required before binding Project
    /// connections. `None` preserves existing agents until they are assigned.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
mod managed_process;
pub use managed_process::ManagedAgentProcess;
