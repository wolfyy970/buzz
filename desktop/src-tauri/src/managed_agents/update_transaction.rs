...
fn ensure_private_anchor(path: &Path) -> Result<(), String> {
...
fn read_owner_file(path: &Path, max_bytes: u64) -> Result<Vec<u8>, String> {
...
fn atomic_write_owner_file(path: &Path, bytes: &[u8]) -> Result<(), String> {
    let parent = path
        .parent()
        .ok_or_else(|| "The update transaction has no private parent.".to_string())?;
...
fn remove_regular_owner_file(path: &Path) -> Result<(), String> {
...
fn classify_invalid_path(path: &Path) -> QuarantineReason {
...
fn recovery_entry_name(entry: &RecoveryJournalEntry) -> &str {
    match entry {
