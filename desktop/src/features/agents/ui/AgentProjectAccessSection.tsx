...
export function AgentProjectAccessSection({
  ...
  const missingRequiredConnections = toolRequirements.filter(
    (requirement) =>
              ...
              <>
                ...
                {missingRequiredConnections.length > 0 ? (
                  <div className="flex flex-wrap items-center justify-between gap-2 rounded-lg bg-muted/40 px-3 py-2">
                    <p className="text-xs text-muted-foreground">
                      Add or test a Project connection to finish this agent
                      setup.
                    </p>
                    <Button
                      disabled={disabled || !projectScope}
        ...
        <Dialog onOpenChange={setConnectionsOpen} open={connectionsOpen}>
          <DialogContent className="flex max-h-[85vh] max-w-2xl flex-col overflow-hidden">
            <DialogHeader className="shrink-0">
              <DialogTitle>Connections for {selectedProject.name}</DialogTitle>
              <DialogDescription>
