...
import {
  ...
  useProjectConnectionImpactMutation,
...
function ConnectionDialog({
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!canSubmit || !project.projectChannelId) return;
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {connection
                ...
                : "Connect a service agents can use in this project. Buzz stores secret values outside the agent template."}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-5">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="analytics-mcp"
                spellCheck={false}
                value={command}
              />
              <p className="text-xs text-muted-foreground">
                Buzz runs this program directly. Shell commands are not
                supported.
              </p>
            </div>
            <button
            ...
            {showTechnicalDetails ? (
              <div className="space-y-4 rounded-xl border border-border/60 bg-muted/15 p-4">
                <div className="space-y-1.5">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Enter one exact argument per line.
                  </p>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Values are stored securely and never shown again.
                      </p>
                    </div>
                    <Button
                      disabled={pending}
...
export function ProjectConnectionsPanel({
  ...
  const impactMutation = useProjectConnectionImpactMutation();
  ...
  const [removalAgents, setRemovalAgents] = React.useState<
    Array<{ pubkey: string; name: string }>
  >([]);
  ...
  async function prepareRemoval(connection: ProjectConnection) {
    try {
      const impact = await impactMutation.mutateAsync(connection.id);
      setRemovalAgents(impact.agents);
      setRemoving(connection);
    } catch (cause) {
      toast.error(
        cause instanceof Error
          ? `Couldn't check which agents use ${connection.name}: ${cause.message}`
          : `Couldn't check which agents use ${connection.name}. Nothing was removed.`,
      );
    }
  }

  async function handleDelete() {
    if (!removing || removalAgents.length > 0) return;
    try {
  ...
  const canAdd = Boolean(project.projectChannelId);
    ...
    <>
        ...
        {!project.projectChannelId ? (
          <div className={PROJECT_DETAIL_PANEL_MESSAGE_CLASS}>
            ...
            <p className="font-medium text-foreground">
              This project needs a discussion channel
            </p>
            <p className="mt-1 text-sm text-muted-foreground">
              Link a discussion channel before connecting services or launching
              agents here.
            </p>
          </div>
        ) : query.isPending ? (
          ...
          <div className={PROJECT_DETAIL_PANEL_MESSAGE_CLASS}>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Add a connection when an agent needs access to analytics, issue
              tracking, or another service.
            </p>
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1">
                        ...
                        if (tested.health.status === "ready") {
                          ...
                          toast.error(
                            tested.health.detail ??
                              `${connection.name} needs attention before an agent can use it.`,
                          );
                        }
                      } catch (cause) {
                        toast.error(
                  ...
                  <Button
                    ...
                    onClick={() => void prepareRemoval(connection)}
                    size="icon-xs"
                    variant="ghost"
                  >
                    <Trash2 className="h-4 w-4" />
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {removalAgents.length > 0
                ? "Connection is still in use"
                : `Remove ${removing?.name ?? "connection"}?`}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {removalAgents.length > 0
                ? `${removalAgents.length} ${removalAgents.length === 1 ? "agent uses" : "agents use"} this connection: ${removalAgents.map((agent) => agent.name).join(", ")}. Edit those agents before removing it.`
                : "Agents will no longer be able to choose this connection. This does not delete data from the connected service."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>
              {removalAgents.length > 0 ? "Close" : "Cancel"}
            </AlertDialogCancel>
            {removalAgents.length === 0 ? (
