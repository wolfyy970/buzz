...
/// Maximum number of structured and legacy MCP servers in one launch.
...
impl std::fmt::Debug for McpConfigDocument {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("McpConfigDocument")
...
impl McpConfigDocument {
    ...
    /// Validate this document, including an optional legacy MCP command.
    pub fn validate(&self, legacy_mcp_command: &str) -> Result<(), McpConfigError> {
        ...
        if self.servers.len() + legacy_count > MCP_SERVER_MAX_COUNT {
            return Err(McpConfigError::Invalid(format!(
        ...
        for (index, server) in self.servers.iter().enumerate() {
            ...
            if legacy_name.as_deref() == Some(name.as_str()) {
                return Err(McpConfigError::Invalid(format!(
                    "MCP server name '{name}' collides with the legacy --mcp-command server"
                )));
            }
            if command.is_empty() || command.contains('\0') {
                return Err(McpConfigError::Invalid(format!(
                    "MCP server '{name}' command must be nonempty and contain no NUL bytes"
                )));
            }
            ...
            for (key, value) in env {
                if !valid_mcp_env_name(key) {
                    return Err(McpConfigError::Invalid(format!(
                        "MCP server '{name}' has invalid environment key '{key}'"
                    )));
                }
                ...
                {
                    return Err(McpConfigError::Invalid(format!(
                        "MCP server '{name}' may not configure protected environment key '{key}'"
                    )));
                }
                if value.contains('\0') {
                    return Err(McpConfigError::Invalid(format!(
                        "MCP server '{name}' environment value for '{key}' contains a NUL byte"
                    )));
                }
            }
        }
    ...
    pub fn to_json(&self, legacy_mcp_command: &str) -> Result<Vec<u8>, McpConfigError> {
        self.validate(legacy_mcp_command)?;
        let encoded =
...
pub fn parse_mcp_config_document(
    ...
    let document: McpConfigDocument = serde_json::from_slice(content)
        .map_err(|error| McpConfigError::Invalid(error.to_string()))?;
    document.validate(legacy_mcp_command)?;
    Ok(document.servers)
}
...
mod tests {
    ...
    fn serialized_document_carries_the_transport_tag() {
        let json =
            serde_json::to_value(McpConfigDocument::new(vec![stdio_server("analytics")])).unwrap();

        assert_eq!(json["version"], MCP_CONFIG_VERSION);
        assert_eq!(json["servers"][0]["transport"], "stdio");
        assert_eq!(
            parse_mcp_config_document(&serde_json::to_vec(&json).unwrap(), "").unwrap(),
    ...
    fn validation_rejects_duplicate_names_and_protected_environment() {
        let duplicate = McpConfigDocument::new(vec![stdio_server("one"), stdio_server("one")]);
        assert!(duplicate.validate("").is_err());

        let protected = McpConfigDocument::new(vec![ConfiguredMcpServer::Stdio {
            name: "one".to_string(),
    ...
    fn debug_output_redacts_environment_values() {
        let output = format!("{:?}", McpConfigDocument::new(vec![stdio_server("one")]));
    ...
    fn encoding_and_parsing_enforce_the_same_size_limit() {
        let oversized = McpConfigDocument::new(vec![ConfiguredMcpServer::Stdio {
            name: "one".to_string(),
            command: "mcp".to_string(),
            args: Vec::new(),
            env: BTreeMap::from([(
        ...
        assert!(
            parse_mcp_config_document(&vec![b' '; MCP_CONFIG_MAX_BYTES as usize + 1], "").is_err()
        );
    }
}
